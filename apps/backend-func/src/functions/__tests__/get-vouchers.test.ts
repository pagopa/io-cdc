...
import { getSession, getVouchers } from "../get-vouchers.js";
...
describe("get-vouchers | getSession", () => {
  afterEach(() => {
    vi.clearAllMocks();
  });
...
describe("get-vouchers | getVouchers", () => {
  afterEach(() => {
    vi.clearAllMocks();
  });

  it("1. should return error when cdc fails", async () => {
    getCdcVouchersTEMock.mockReturnValueOnce(TE.left(new Error("cdc error")));
    const res = await getVouchers(aValidSession)(deps)();
    expect(E.isLeft(res)).toBe(true);
    if (E.isLeft(res))
      expect(res.left).toEqual({
        code: 500,
